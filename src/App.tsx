...
export const App: React.FC = () => {
  ...
  // Global Android Back Button Dispatcher Setup
  useEffect(() => {
    (window as any).handleAndroidBack = () => {
      return backButtonManager.handleBack();
    };
  ...
  // Back button registrations for App-level modals and tabs
  useEffect(() => {
    if (isTourOpen) {
      backButtonManager.register('app-tour-modal', () => {
        setIsTourOpen(false);
        return true;
      }, 50);
    } else {
      backButtonManager.unregister('app-tour-modal');
    }
  }, [isTourOpen]);

  useEffect(() => {
    if (isFilterDrawerOpen) {
      backButtonManager.register('app-filter-drawer', () => {
        setIsFilterDrawerOpen(false);
        return true;
      }, 50);
    } else {
      backButtonManager.unregister('app-filter-drawer');
    }
  }, [isFilterDrawerOpen]);

  useEffect(() => {
    if (isWidgetModalOpen) {
      backButtonManager.register('app-widget-modal', () => {
        setIsWidgetModalOpen(false);
        return true;
      }, 50);
    } else {
      backButtonManager.unregister('app-widget-modal');
    }
  }, [isWidgetModalOpen]);

  useEffect(() => {
    if (isAccountModalOpen) {
      backButtonManager.register('app-account-modal', () => {
        setIsAccountModalOpen(false);
        return true;
      }, 50);
    } else {
      backButtonManager.unregister('app-account-modal');
    }
  }, [isAccountModalOpen]);
  ...
  useEffect(() => {
    if (activeTab !== 'timeline') {
      ...
      backButtonManager.unregister('app-tab-navigation');
    }
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    notificationService.startScheduler(
      ...
      updater => {
        ...
        notificationService.syncNativeReminder(next);
      }
    );

    // Sync native scheduled alarm on launch
    notificationService.syncNativeReminder(storageService.getReminder());
  ...
  const todayExpense = useMemo(() => {
    ...
    const monthPrefixToday = todayStr.slice(0, 7);

    let sum = 0;
    transactions.forEach(t => {
      if (t.type === 'expense') {
        if (t.isAmortized) {
          if (t.date.startsWith(monthPrefixToday)) {
            sum += t.amount / daysInMonth;
          }
        } else {
          if (t.date === todayStr) {
            sum += t.amount;
  ...
  // 7-day daily expense trend for Android widgets & widget center (with amortization support)
  const recent7DaysExpenses = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const daysInMonth = new Date(y, m, 0).getDate();
      const monthPrefix = ds.slice(0, 7);
