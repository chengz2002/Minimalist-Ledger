    ...
    if (timeRange === 'month') {
      ...
      transactions.forEach(t => {
        if (t.type === statType && t.date.startsWith(monthPrefix)) {
          if (t.isAmortized && statType === 'expense') {
            const dailySlice = t.amount / daysInMonth;
            for (let d = 0; d < daysInMonth; d++) {
              days[d].amount += dailySlice;
            }
          } else {
      ...
      transactions.forEach(t => {
        if (t.type === statType) {
          if (t.isAmortized && statType === 'expense') {
            ...
            const daysInTMonth = new Date(tYear, tMonth, 0).getDate();
            const dailySlice = t.amount / daysInTMonth;
            const tMonthPrefix = `${tYear}-${String(tMonth).padStart(2, '0')}`;

            currentWeekDays.forEach((w, idx) => {
              if (w.dateStr.startsWith(tMonthPrefix)) {
                days[idx].amount += dailySlice;
              }
            });
          } else {
