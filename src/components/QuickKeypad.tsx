...
export const QuickKeypad: React.FC<QuickKeypadProps> = ({
  ...
  // Date formatted for quick pill with weekday
  const getShortDateLabel = (dateStr: string) => {
    ...
    const weekName = weekLabels[dayOfWeek];

    if (dateStr === today) return `今天 (${weekName})`;
    if (dateStr === yesterday) return `昨天 (${weekName})`;
    return `${dateStr.slice(5)} ${weekName}`; // MM-DD 周几
  };
  ...
  const dailyAmortizedAmount = computedAmount > 0 ? (computedAmount / daysInCurrentMonth).toFixed(2) : '0.00';
